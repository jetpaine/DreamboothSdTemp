...
class JoePennaDreamboothConfigSchemaV1:
    def __init__(self):
        ...
        # Checkpoint Format (ckpt or safetensors)
        self.checkpoint_format: str = 'ckpt'
    ...
    def saturate(
        ...
        if checkpoint_format.lower() not in ['ckpt', 'safetensors']:
            raise Exception("'--checkpoint_format': Must be either 'ckpt' or 'safetensors'.")
    ...
    def saturate_from_file(
        ...
        if not os.path.exists(config_file_path):
            ...
            if config_parsed['schema'] == 1:
                # Handle checkpoint_format for backward compatibility
                checkpoint_format = 'ckpt'
                if 'checkpoint_format' in config_parsed:
                    checkpoint_format = config_parsed['checkpoint_format']
    ...
    def create_checkpoint_file_name(self, steps: str):
        ...
        extension = self.checkpoint_format
        return f"{date_string}_{self.project_name}_" \
               ...
               f"{self.class_word}_class_word.{extension}".replace(" ", "_")
